...
public class AdminCouponController {
    ...
    public Object list(String name, Short type, Short status,
        ...
        List<MallCoupon> couponList = mallCouponService.querySelective(name, type, status, page, limit, sort, order);
    ...
    public Object listuser(Integer userId, Integer couponId, Short status,
                           ...
                           @Sort @RequestParam(defaultValue = "add_time") String sort,
                           @Order @RequestParam(defaultValue = "desc") String order) {
        List<MallCouponUser> couponList = mallCouponUserService.queryList(userId, couponId, status, page,
                limit, sort, order);
        return ResponseUtil.okList(couponList);
    }
    ...
    public Object read(@NotNull Integer id) {
        MallCoupon coupon = mallCouponService.findById(id);
        return ResponseUtil.ok(coupon);
    }
    ...
    public Object update(@RequestBody MallCoupon coupon) {
        ...
        if (mallCouponService.updateById(coupon) == 0) {
    ...
    public Object delete(@RequestBody MallCoupon coupon) {
        mallCouponService.deleteById(coupon.getId());
